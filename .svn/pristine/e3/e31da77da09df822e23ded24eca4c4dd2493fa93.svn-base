...
class WechatAuthService
{
    ...
    public function getAccessToken()
    {
        ...
        $sKey = $this->appId . '_client_AccessToken';
        $accessToken = $S->get($sKey);
        ...
        if (is_array($token)) {
            if (isset($token['errcode'])) {
                ...
                $S->set($sKey, $token['access_token'], 7000);
    ...
    /**
     * 获取jsapi_ticket
     * @return mixed jsapi_ticket
     * @throws \Exception
     */
    private function getJsApiTicket()
    {
        $S = Redismem::getInstance();
        $sKey = $this->appId . '_Jsapi_Ticket';
        $ticket = $S->get($sKey);
        if (!empty($ticket)) {
            return $ticket;
        }
    ...
    /**
     * 上传零时媒体资源
     * @param $filename 媒体资源本地路径
     * @param $type 媒体资源类型，具体请参考微信开发手册
     * @return array
     * @throws \Exception
     */
     ...
     * @param $content 发送的数据，不同类型的数据结构可能不同
    ...
    /**
     * 给指定用户推送模板信息
     * 注意：微信规则只允许给已关注公众平台的用户推送信息
     * @param  string $openid  用户的openid
     * @param  string $templateId 模板id
     * @param  string $url
     * @param  string $topcolor    
     * @param  array  $template 发送的模板，不同类型的模板结构可能不同
     */
    public function messageTemplateSend()
    {
        $args = func_get_args();
        $template = array_pop($args);
    ...
    /**
     * 创建自定义菜单
     * @param $button $button 符合规则的菜单数组，规则参见微信手册
     * @return array
     */
